...
function renderLists(){
  $('#productsList').innerHTML=prods.map(p=>`<div class="admin-row"><img src="${esc(p.image_url||'')}" alt=""><div><b>${esc(p.name)}</b><small> — ${esc(p.country||'')} — ${Number(p.price||0).toLocaleString('ar-SA')} ر.س — ${p.active?'ظاهر':'مخفي'}</small></div><div class="admin-row-actions"><button class="edit-btn" onclick="editP('${p.id}')">تعديل</button><button class="delete-btn" onclick="delP('${p.id}')">حذف</button></div></div>`).join('');
...
$('#productForm').onsubmit=async e=>{e.preventDefault();const id=$('#productId').value;let image=$('#pImage').value.trim();const file=$('#pFile').files[0];if(file)image=await uploadImage(file);if(!image)return toast('أضف صورة أو رابط صورة',true);const direct=$('#pDirectOrder').checked,affiliate=$('#pAffiliate').value.trim();if(!direct&&!affiliate)return toast('أضف رابط الشراء أو فعّل الطلب عبر واتساب',true);const row={name:$('#pName').value,category_id:$('#pCategory').value,country:$('#pCountry').value,price:Number($('#pPrice').value||0),image_url:image,affiliate_url:affiliate,description:$('#pDescription').value,active:$('#pActive').checked,direct_order:direct};let r=id?await sb.from('products').update(row).eq('id',id):await sb.from('products').insert(row);if(r.error)toast(r.error.message,true);else{toast('تم حفظ المنتج');e.target.reset();$('#productId').value='';$('#pActive').checked=true;$('#pDirectOrder').checked=false;loadAll()}};
